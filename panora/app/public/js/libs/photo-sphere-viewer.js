...
PhotoSphereViewer.prototype.run = function(){
        let that = this;
...
PhotoSphereViewer.prototype._createScene = function() {
  ...
  this.camera = new THREE.PerspectiveCamera(this.config.default_fov, this.prop.size.width / this.prop.size.height, 0.01, cameraDistance);
...
PhotoSphereViewer.prototype.render = function(updateDirection) {
  ...
  if(typeof this.updateLabel == 'function'){
    ...
    this.updateLabel(this.sphericalCoordsToVector3(this.prop));
  }
  if (this.config.fisheye) {
...
PhotoSphereViewer.prototype.setPanorama = function(path, position, transition) {
  ...
  if (!transition || !this.config.transition || !this.scene) {
    this.loader.show();
    if (this.canvas_container) {
      //this.canvas_container.style.opacity = 0;原生的样式
      this.canvas_container.style.opacity = 1;//修改后的苏俊凌
    }
